...
struct DBEngine {
  ...
  struct FileHandler {
    ...
    static void createUntypedTable(CreateUntypedTableNode& node, ExecutionContext& ctx) {
      ...
      if (!file.is_open()) {
        std::cerr << "Error: Could not create metadata file for table " << node.tableName << "\n";
        return;
      }
      ...
      std::cout << "Created table " << node.tableName << " with " << cols.size() << " columns\n";
...
const std::string DBEngine::FileHandler::METADATA_BASE_DIRECTORY =
    DBEngine::FileHandler::SCHEMA_BASE_DIRECTORY + "/metadata";
const std::string DBEngine::FileHandler::DATASTORAGE_BASE_DIRECTORY =
    DBEngine::FileHandler::SCHEMA_BASE_DIRECTORY + "/data";
