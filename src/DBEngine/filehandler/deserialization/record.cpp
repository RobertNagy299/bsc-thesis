...
DB_Types::TableFileDeserializationIndicator FileHandler::Deserializer::deserializeNextRecordAndMarkAsTombstone(
    std::fstream& table_file, const std::vector<UntypedColumnDefNode*>& schema, const WhereNode* where,
    DB_Types::Record& out_record) {
  ...
  // at this point, this is a live record - let's skip the column offset region
  std::uint64_t number_of_columns_without_pk = schema.size() - 0x1ul;
  std::uint64_t col_offset_region_length = number_of_columns_without_pk * sizeof(DB_Types::column_offset_t);
  table_file.seekg(col_offset_region_length, std::ios::cur);
  // At this point, the seekg pointer should be at the beginning of the primary key region
  ...
  // default is true, because if no where clause was given, we want to update everything
  bool where_result = true;
  ...
  for (std::size_t i = 0; i < schema.size(); ++i) {
    ...
    // --- projection will be done in the executor ---
    out_record.push_back(std::move(deserialized_value));
  }

  // at this point, the seekg pointer should point to the beginning of the next record and we have a deserialized
  // record, but we only want to actually process it if the where condition holds, or if the where clause was omitted

  // signal to the executor that this record shouldn't be added to the final result set
  // because it doesn't meet the where condition
  if (!where_result) { return DB_Types::TableFileDeserializationIndicator::SKIP; }

  // indicate that a live record was found and updated - mark the old record as a tombstone
